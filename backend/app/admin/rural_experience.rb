ActiveAdmin.register Tourism, as: "RuralExperience" do
  extend PartnerAdmin
  initialize_entity_record

  permit_params Tourism.admin_permitted_params
  menu label: I18n.t('active_admin.rural_experience'),
    priority: 6,
    url: "/admin/#{I18n.t('routes.rural_experiences')}"
  ...
  scope (I18n.t('active_admin.incorporated').pluralize) { Tourism.rural_experience.incorporated }
  scope (I18n.t('active_admin.disabled').pluralize) { Tourism.disabled.rural_experience }
  ...
  index do
    ...
    actions do |partner|
      if current_user.has_role?(:admin)
        if partner.disabled?
          item I18n.t('active_admin.enable'), disable_admin_rural_experience_path(partner), method: :put
        else
          item I18n.t('active_admin.disable'), disable_admin_rural_experience_path(partner), method: :put
        end
      end
    end
  end
  ...
  filter :with_products, label: Tourism.human_attribute_name(:product_list), as: :string
  ...
  member_action :disable, method: :put do
    ...
    redirect_to admin_rural_experiences_path, notice: "#{ notice }!"
  end

  controller do
    def scoped_collection
      Tourism.rural_experience
    end
  end
